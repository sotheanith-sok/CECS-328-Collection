...
int main() {
	...
	double totalTimeForSelectionSort = 0;
	for (int i = 0; i < 100; i++) {
		std::vector<int> A;
		...
		t2 = std::chrono::high_resolution_clock::now();
		std::cout << i << std::endl;
		totalTimeForSelectionSort += (std::chrono::duration<double, std::nano>(t2 - t1)).count();
	}
	std::cout << "Average running for HeapSort: " << (totalTimeForHeapSort / 100) <<" nanosecond"<< std::endl;
	std::cout << "Average running for Selection Sort: " << (totalTimeForSelectionSort / 100) << " nanosecond" << std::endl;
}
std::vector<int> build_MaxHeap(std::vector<int> A) {
